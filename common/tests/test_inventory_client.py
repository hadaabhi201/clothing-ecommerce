import pytest
from httpx import Response, Request, RequestError, TimeoutException
...
def mock_inventory_api_settings(monkeypatch):
    mock_settings = InventoryAPIConfig(
        INVENTORY_BASE_URL="http://mock-inventory-api",
...
async def test_find_item_success():
    ...
    with patch("common.inventory_client.inventory_client.httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
